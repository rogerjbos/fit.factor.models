...
All the necessary data is passed to *fitdata* and the user also has to pass in all the important column names, such as date (`date.var`), id (`id.var`), returns (`return.var`), and exposures (`exposure.vars`).  It is usual for the factors to be cross-sectional standardized so they have a mean of zero and a standard deviation of one for each time period.  This aids in comparability over time.  The *fit.attribution* function can do this for you by setting `z.score = TRUE`.  In our example we set `z.score = FALSE` because the data is already standardized outside of the function.  You could standardize the returns as well, but we don't do that because then you lose the interpret-ability (the output would be in standard deviation terms instead of raw returns).  The option `full.resid.cov = TRUE` estimates the full residual covariance matrix, but this calculation can be slow.  Setting this option to false returns the diagonal matrix and is much quicker. 
...
We can fit a fundamental risk model as follows.

```
fit.fundamental(retMat, factor.loadings, calc.inv = FALSE)
```
...
r1 <- fit.data.cast(balanced, item='retVar', id.var = 'gvkey', date.var = 'datadate', reverse = TRUE)
r1[is.na(r1)] <- 0
head(r1)
```


For the sake of simplicity, we can use the factor loadings matrix returned by the _fit.attribution()_ function.
...
Once we have all the necessary ingredients, we can estimate the fundamental risk model and see what is returned.

```{r}
# Example 7

riskmod <- fit.fundamental(retMat = as.matrix(r1), factor.loadings = load, calc.inv = FALSE)
names(riskmod)
```
...
riskmod <- fit.statistical(r1, use.cor = TRUE)
names(riskmod)
```
...
#### Sector / Group / Industry Classification

There is also a function that uses the R function `kmeans` to determine sector, group, and industry classifications. `kmeans` is a K-nearest neighbor algorithm that classifies objects based on average distance to centers (`k` specifies the number of centers).  The user specifies how many centers (classifications) they want and kmeans attempts to find the centers that result in grouping the observations nearest to those centers.  The number of centers returns may be less than the specified k, but will never be more than k.

As an example, we will load some price data for 123 ETFs to classify them into sectors, groups, and industries.

```{r echo=FALSE, results="hide"}

load("/media/research/R_HOME/etfs/fma_rawdata.Rdata")

initDate <- "2000-01-01"
symbols <- as.character(unique(fma_rawdata$ETF))
idx <- symbols %in% ls()
newsym <- symbols[!idx] 
go <- try(getSymbols(newsym, from = initDate))
...
for(i in 1:length(symbols)) {
  ai <- try(Ad(get(symbols[i])), silent=TRUE)
  oi <- try(Op(get(symbols[i])), silent=TRUE)
  hi <- try(Hi(get(symbols[i])), silent=TRUE)
  li <- try(Lo(get(symbols[i])), silent=TRUE)
  ci <- try(Cl(get(symbols[i])), silent=TRUE)
  vi <- try(Vo(get(symbols[i])), silent=TRUE)
  names(ai) <- names(oi) <- names(hi) <- names(li) <- names(ci) <- names(vi) <- symbols[i]
  adjusted[[i]] <- ai
  open[[i]] <- oi
  high[[i]] <- hi
  low[[i]] <- li
...
adjusted <- t(do.call(cbind, adjusted))
adjusted <- adjusted[, ncol(adjusted):1]

open <- t(do.call(cbind, open))
open <- open[, ncol(open):1]

high <- t(do.call(cbind, high))
high <- high[, ncol(high):1]

low <- t(do.call(cbind, low))
low <- low[, ncol(low):1]

close <- t(do.call(cbind, close))
close <- close[, ncol(close):1]

ret <- t(do.call(cbind, ret))
ret <- ret[, ncol(ret):1]
ret[is.na(ret)] <- 0

volume <- t(do.call(cbind, volume))
volume <- volume[, ncol(volume):1]

i <- 1
retMat <- as.matrix(ret[, i:(i + 252)])

k <- c(30, 20, 10)
ind.list <- fit.stat.ind.class.all(retMat = retMat, 
                                   k = k, 
                                   iter.max = 100, 
                                   num.try = 100,
                                   do.demean = c(FALSE, FALSE, FALSE),
                                   norm.cl.ret = TRUE)
ind <- ind.list[[1]]
grp <- ind.list[[2]]
sect <- ind.list[[3]]
knitr::kable(showIndustry(sect, symbols))

ind.list <- fit.cluster.all(retMat = retMat, k = k)
ind2 <- ind.list[[1]]
grp2 <- ind.list[[2]]
sect2 <- ind.list[[3]]
knitr::kable(showIndustry(sect2, symbols))


ind.list <- fit.cluster.all(retMat = retMat, k = k, method="rcpp")
ind3 <- ind.list[[1]]
grp3 <- ind.list[[2]]
sect3 <- ind.list[[3]]
knitr::kable(showIndustry(sect3, symbols))

ind.list <- fit.cluster.all(retMat = retMat, k = k, method="medoids")
ind4 <- ind.list[[1]]
grp4 <- ind.list[[2]]
sect4 <- ind.list[[3]]
knitr::kable(showIndustry(sect4, symbols))

# ind <- fit.stat.ind.class(retMat = retMat, k = 3)
# mh(ind)
# ind.cluster <- fit.cluster.kmeans(retMat = retMat, k = 3)
# mh(ind.cluster)
# ind.rcpp <- fit.cluster.rcpp(retMat = retMat, k = 3)
# mh(ind.rcpp)
# ind.me <- fit.cluster.medoids(retMat = retMat, k = 3)
# mh(ind.me)

#mh(ind.list)

# ind <- ind.list[[3]]
# showIndustry(ind, symbols)

# ind.list <- fit.stat.ind.class.dyn(retMat = retMat,
#                                    p = 2,
#                                    iter.max = 10,
#                                    num.try = 100,
#                                    top.down = TRUE)
...
# e <- fit.calc.eigen(retMat = retMat,
#                            k = 10,
#                            prec = .001)


# ind.list <- fit.stat.clust.all(retMat = retMat,
#                                k = k,
#                                do.demean = rep(FALSE, length(k)),
#                                norm.cl.ret = TRUE)

# ind.list.imp  <- fit.improve.ind.class(retMat = retMat,
#                                        ind = ind.list[[1]],
#                                        iter.max = 10,
#                                        num.try = 100)
# mh(ind.list.imp)  
...
# rr <- fit.general.heterotic(retMat = ret, ind = ind.list, mkt.fac = mkt.fac, rm.sing.tkr = rm.sing.tkr, p = p,
#                             append.style = append.style, k.style = k.style, style = style)
# rr <- fit.heterotic(retMat = ret, ind = ind.list, mkt.fac = TRUE, rm.sing.tkr = TRUE) 
# 
# names(rr)
# mh(rr$specific.risk)
# mh(rr$factor.cov)
# mh(rr$factor.loadings)
# mh(rr$cov.mat)
# mh(rr$inverse.cov)

```  
  
We downloaded daily prices for the 80+ ETFs and calculated the daily returns.  Here is a sample of the returns matrix for the most recent one year of data.
...
We use the function *fit.stat.ind.class.all* to perform the k-means classification.  The only data it needs is the return matrix.  One year of daily data should be sufficient to get reasonable results.  We use the variable `k` to specify how many groupings we want.  `k` can be a scalar (if you only want one grouping) or a vector (if you want multiple groupings).  We will ask for three groupings: 30, 20, and 10.  We will call them industry, group, and sector.
...
ind.list <- fit.stat.ind.class.all(retMat = retMat, 
                                   ...
                                   norm.cl.ret = TRUE)
```
Here is a sample of the resulting industry assignment.

```{r}
ind <- ind.list[[1]]
rownames(ind) <- symbols
dim(ind)
head(ind)
```
We asked for 30 industries and we got `r dim(ind)[2]`.

Here is a sample of the resulting group assignment.

```{r}
grp <- ind.list[[2]]
rownames(grp) <- symbols
dim(grp)
head(grp)
```
We asked for 20 groups and we got `r dim(grp)[2]`.
  
Here is a sample of the resulting sector assignment.
...
head(sect)
```
We asked for 10 sectors and we got `r dim(sect)[2]`.
  
How good are these groupings?  Are they even reasonable?  To try to answer this question, we created a simple function that, for each security (ETF in this case), calculates the average correlation of that security to all others in the same group (_Cor_same_) and calculates the average correlation of that security to all the others in the different groups (_Cor_other_).  As expected, the average correlations in the _Cor_same_ column are higher (sometimes much higher) than the average correlations in the _Cor_other_ column.

```{r}
# Example 11

showIndustry <- function(ind, symbols) {
  out <- list()
  for (i in 1:ncol(ind)) {
    ...
    if (n > 1) {
      ids <- symbols[ii]
      ri <- retMat[ii, ]
      same_sector <- cor(t(ri), t(ri))
      other_sector <- cor(t(retMat[ii==FALSE, ]), t(ri))
      Cor_same <- colMeans(same_sector)
      Cor_other <- colMeans(other_sector)
      tmp <- data.frame(Industry=rep(i, n), symbol=ids, Cor_same, Cor_other)
      out[[i]]  <- tmp
    }
  }
  out <- rbindlist(out)
  out <- rbind(out, data.frame(Industry=0, symbol="average", Cor_same=g(mean(out$Cor_same)), Cor_other=g(mean(out$Cor_other))))
  out
...
In this brief vignette we showed examples of portfolio attribution using custom factors.  We also showed how to calculate fundamental and statistical risk models using your custom factors.  Finally, we showed how to use k-means classification to derive industry assignments, even for instruments like ETFs that don't have natural industry assignments.  Even though assigning ETFs to industries may seem questionable, we show that the average correlation of ETFs within the same sector in which it was assigned is higher than the average correlation to ETFs in all the other sectors.  There is a lot more that can be done with this *fit factor model* package and we hope this has sparked some ideas on your end.
...
$\Gamma$ (specific.risk) is a vector of length N,
...
$$ Expo_{port} = W_{port} * \beta $$

#### Benchmark Exposure
$$ Expo_{bench} = W_{bench} * \beta $$
...
$$ AbsoluteRisk_{port} = \sqrt{ Expo_{port} * \Delta * Expo_{port}^T + \sum{W_{port}^2 * \Gamma^2} } $$
$$ AbsoluteRisk_{bench} = \sqrt{ Expo_{bench} * \Delta * Expo_{bench}^T + \sum{W_{bench}^2 * \Gamma^2} } $$


#### Marginal Factor Variance
$$ MarginalFactorVariance = 2 * \Delta * Expo_{bench}^T $$
...
$$ FactorRisk_{port} = W_{active} * .5 * MarginalFactorVariance$$
$$ FactorRisk_{port} = \sum{FactorRisk_{security}}$$
$$ FactorRisk_{port} = W_{active} * \beta * \Delta * W_{active}^T * \beta $$

#### Stock Specific Risk (Variance)
$$ SSR_{security} = W_{active}^2 * \Gamma^2 $$
$$ SSR_{port} = \sum{SSR_{security}} $$

#### Total Risk (Variance)
$$ TotalRisk_{security} = FactorRisk_{port} + SSR_{security} $$
$$ TotalRisk_{port} = FactorRisk_{port} + SSR_{port} $$
...
$$ TrackingError_{predicted} = \sqrt{TotalRisk_{port}} $$

#### R-Squared
This measures the percent of the portfolio variance explained by the benchmark.
$$ Rsquared = AbsoluteRisk_{port}^2 + AbsoluteRisk_{bench}^2 - TrackingError_{predicted}^2) / 2) / (AbsoluteRisk_{port} + AbsoluteRisk_{bench}))^2 $$

#### Percent Factor Risk
These measures allow us to break down the factor risk by security and by factor.
$$ PercentFactorRisk_{security} = FactorRisk_{security} / TotalRisk_{port} $$
$$  PercentFactorRisk_{port} = FactorRisk_{port} / TotalRisk_{port} $$
$$  FactorRisk_{factor} = W_{active} * \beta * \Delta * W_{active} * \beta $$
$$  PercentFactorRisk_{factor} = FactorRisk_{factor} / TotalRisk_{port} $$

### Contribution to Tracking Error
$$  contributionTE_{security} = TotalRisk_{security} / TotalRisk_{port} * TrackingError_{predicted} $$

#### Contribution to Tracking Error - Factor Contribution
$$  contributionTE_{factor} = FactorRisk_{security} / TotalRisk_{port} * TrackingError_{predicted} $$

#### Contribution to Tracking Error - Stock Specific Risk
$$  contributionTE_{SSR} = SSR_{security} / TotalRisk_{port} * TrackingError_{predicted} $$

#### Predicted Beta of the portfolio
Given portfolio weights and benchmark weights, you can calculate the predicted beta by using the standard formula for beta and plugging in the outputs from the risk model: $\beta$, $\Delta$, and $\Gamma$.
$$ PredictedBeta = \frac{W_{port}^T * \beta * \Delta * \beta^T * W_{bench} + W_{port}^T  * \Gamma * \Gamma * W_{bench}}{W_{bench}^T * \beta * \Delta * \beta^T * W_{bench} + W_{bench}^T * \Gamma * \Gamma * W_{bench}} $$
