...
cfit$returns.table$Residual[2]
cfit$returns.table$Portfolio.Return[2]
...
r1 <- fit.data.cast(stock, item='retVar', id.var = 'gvkey', date.var = 'datadate', reverse = TRUE)
r1[is.na(r1)] <- 0
head(r1)
```
...
$$ AbsoluteRisk_{portfolio} = \sqrt{ Expo_{portfolio} * \Delta * Expo_{portfolio}^T + \sum{W_{portfolio}^2 * \Gamma^2} } $$
$$ AbsoluteRisk_{benchmark} = \sqrt{ Expo_{benchmark} * \Delta * Expo_{benchmark}^T + \sum{W_{benchmark}^2 * \Gamma^2} } $$
...
$$ PredictedBeta = \frac{W_{portfolio}^T * \beta * \Delta * \beta^T * W_{benchmark} + W_{portfolio}^T  * \Gamma * \Gamma * W_{benchmark}}{W_{benchmark}^T * \beta * \Delta * \beta^T * W_{benchmark} + W_{benchmark}^T * \Gamma * \Gamma * W_{benchmark}} $$
